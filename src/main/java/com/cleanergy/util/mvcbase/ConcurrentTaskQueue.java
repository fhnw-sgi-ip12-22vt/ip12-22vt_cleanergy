 ...
 * <p>
 * Execution is asynchronous - possibly in a different thread - but the sequence is kept stable,
 * such that for all tasks A and B: if B is submitted after A, B will only be executed after A is
 * finished.
 * <p>
 * New tasks can be submitted while tasks are running.
 * <p>
 * Task submission itself is supposed to be thread-confined, i.e. creation of the
 * ConcurrentTaskQueue and task submission is expected to run in the same thread, most likely the
 * JavaFX UI Application Thread.
 *
 * @author Dierk Koenig
 */
...
public final class ConcurrentTaskQueue<R> {
  ...
  public ConcurrentTaskQueue(Duration maxToDoTime) {
    ...
    this.executor = Executors.newFixedThreadPool(
        1);  // use 2 for overlapping onDone with next to-do
