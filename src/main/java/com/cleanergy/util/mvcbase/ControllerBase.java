 ...
 * <p>
 * Execution of Actions is asynchronous. The sequence is kept stable, such that for all actions A
 * and B: if B is submitted after A, B will only be executed after A is finished.
 */
public abstract class ControllerBase<M> {
  ...
  /**
   * Only the other base classes 'ViewMixin' and 'PUI_Base' need access, therefore it's 'package
   * private'
   */
  M getModel() {
    return model;
   ...
   * <p>
   * An {@link InterruptedException} will be catched and ignored while setting the interrupt flag
   * again.
   *
