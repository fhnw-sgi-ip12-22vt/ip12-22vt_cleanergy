package com.cleanergy.components;
...
public class SimpleButton extends Component {
  ...
  /**
   * Specifies if button state is inverted, e.g. HIGH = depressed, LOW = pressed This will also
   * automatically switch the pull resistance to PULL_UP
   */
   ...
   */
  private DigitalInputConfig buildDigitalInputConfig(Context pi4j, PIN address, boolean inverted,
      long debounce) {
  ...
  /**
   * Sets or disables the handler for the onDown event. This event gets triggered whenever the
   * button is pressed. Only a single event handler can be registered at once.
   *
  ...
  /**
   * Sets or disables the handler for the onUp event. This event gets triggered whenever the button
   * is no longer pressed. Only a single event handler can be registered at once.
   *
   * @param task Event handler to call or null to disable
   */
  ...
  /**
   * Sets or disables the handler for the whilePressed event. This event gets triggered whenever the
   * button is pressed. Only a single event handler can be registered at once.
   *
   * @param task Event handler to call or null to disable
   */
